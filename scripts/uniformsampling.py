...
def collect_samples():
    ...
    for i in range(20000):
        env.reset()
        curr_0 = random.uniform(-2.3,2.3)
        curr_1 = random.uniform(-2.5,2.5)
        curr_2 = random.uniform(-0.25,0.25)
        curr_3 = random.uniform(-2.5,2.5)
        curr = [curr_0, curr_1, curr_2, curr_3]
        curr = np.array(curr)
        env.env.state = curr
...
#model = Network().to(device)
#model.load_state_dict(torch.load("working"))
#target = Network().to(device)
#target.load_state_dict(model.state_dict())
#target.eval()
# optimizer = optim.Adam(model.parameters(), LR)
train(500, switch = True, test = False, reset_episode_durations = True)
train(1000, switch = False, test = True, reset_episode_durations = True)

# torch.save(model.state_dict(), "working")
